...
def get_visualization_params(satellite, index, min_val=None, max_val=None, palette=None):
    """
    ...
    if satellite == 'Landsat' or satellite == 'landsat':
        ...
        raise ValueError("Incorrect definition of satellite. Options: 'Landsat', 'landsat', 'Sentinel2', or 'sentinel2'")

    # Get default visualization parameters for the given image name
    params = params_dict.get(index, None)
    if params is None:
        raise ValueError("Incorrect definition of index. Options: 'TrueColor', 'NDVI', 'NDWI', 'halite', 'gypsum', 'LST', 'NDTI', or 'KIVU'. Note LST is not available for Sentinel2")
